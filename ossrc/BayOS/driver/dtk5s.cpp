...
typedef dword tek_TPRB;

static unsigned int tek_getnum_s7s(byte **pp);
static int tek_lzrestore_tek5(int srcsiz, byte *src, int outsiz, byte *outbuf);
static int tek_decmain5(int *work, byte *src, int osiz, byte *q, int lc, int pb, int lp);

int tek_checkformat(int siz, byte *p)
{
    static byte header[] = "\xff\xff\xff\x01\x00\x00\x00" "OSASKCMP";
    int i;
    if (siz < 17)
        return -1;
    for (i = 0; i < 15; i++)
...
int tek_decode(int siz, byte *p, byte *q)
{
    byte *p1 = p + siz;
...
static int tek_lzrestore_tek5(int srcsiz, byte *src, int outsiz, byte *outbuf)
{
    int wrksiz, lc, lp, pb, *work, prop0, fl, result;
    ...
    result = tek_decmain5(work, src, outsiz, outbuf, lc, pb, lp);
    free(work);
    return result;
}
...
shift:
    ...
    while (rd->range < (dword) (1 << 24));
    goto shift1;
}

static int tek_rdget1(struct tek_STR_RNGDEC *rd, dword *prob0, int n, int j)
{
    dword i, *prob;
    prob0 -= j;
    do
...
static dword tek_revbit(dword data, int len)
{
    dword rev = 0;
    do
    {
...
static int tek_decmain5(int *work, byte *src, int osiz, byte *q, int lc, int pb, int lp)
{
    ...
    for (i = 0; i < 4; i++)
        rep[i] = ~i;
    for (i = sizeof (struct tek_STR_PRB) / sizeof (dword) + (0x300 << (lc + lp)) - 2; i >= 0; i--)
        ((dword *) prb)[i] = 1 << 10;
    ...
    *q++ = tek_rdget1(rd, prb->lit, 8, 1) & 0xff;
    ...
    {
        s_pos = pos & m_pos;
        if (tek_rdget1(rd, &prb->pb[s_pos].st[s].mch, 1, 0) == 0)
        { /* 非lz */
        ...
        { /* lz */
            pmch |= 1;
            if (tek_rdget1(rd, &prb->st[s].rep, 1, 0) == 0)
            { /* len/dis */
                rep[3] = rep[2];
                rep[2] = rep[1];
                rep[1] = rep[0];
            ...
            { /* repeat-dis */
                if (tek_rdget1(rd, &prb->st[s].repg0, 1, 0) == 0)
                { /* rep0 */
                    i |= -1;
