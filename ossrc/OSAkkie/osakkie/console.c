	...
	for (;;) {
		...
		if (fifo32_status(&task->fifo) == 0) {
			...
			if (i == 3) {
				/* カーソルOFF */
				if (cons.sht != 0) {
					boxfill8(cons.sht->buf, cons.sht->bxsize, COL8_000000, cons.curx, cons.cury, cons.curx + 7, cons.cury + 15);
				}
				cons.curcol = -1;
			}
			if (i == 4) {
			...
			if (cons.sht != 0) {
				if (cons.cury >= 0) {
					boxfill8(cons.sht->buf, cons.sht->bxsize, cons.curcol, cons.curx, cons.cury + 15, cons.curx + 7, cons.cury + 15);
				}
...
void cmd_wallpaper(struct CONSOLE *cons, char *cmdline, int *fat)
{
	...
	struct RGB *picbuf, *q;
	int i, j, info[8], fsize;
	char *filebuf, *p;

	if (cmdline[10] == '0' && cmdline[11] == 0) {
		/* 壁紙OFF */
		init_screen8(sht->buf, sht->bxsize, sht->bysize);
	} else {
		/* 壁紙表示 */
		finfo = file_search(cmdline + 10, (struct FILEINFO *) (ADR_DISKIMG + 0x002600), 224);
		if (finfo != 0) {
			/* 画像ファイルが見つかったときの処理 */
			fsize = finfo->size;
			filebuf = file_loadfile2(finfo->clustno, &fsize, fat);
			i = picture_info(&env, info, fsize, filebuf);
			if (i == 0) {
				cons_putstr0(cons, "File is not picture.\n");
				return;
			}
			picbuf = (struct RGB *) memman_alloc_4k(memman, info[2] * info[3] * sizeof(struct RGB));
			i = picture_decode0(info[0], &env, fsize, filebuf, 4, (unsigned char *) picbuf, 0);
			if (i != 0) {
				cons_putstr0(cons, "Picture decode error.\n");
				return;
			}
			/* 表示 */
			if ((info[2] < sht->bxsize) | (info[3] < sht->bysize)) {
				/* 画面サイズよりも画像が小さかったら背景色で塗りつぶす */
				init_screen8(sht->buf, sht->bxsize, sht->bysize);
			}
			for (i = 0; i < info[3]; i++) {
				p = sht->buf + ((sht->bysize - info[3]) / 2 + i) * sht->bxsize + (sht->bxsize - info[2]) / 2;
				q = picbuf + i * info[2];
				for (j = 0; j < info[2]; j++) {
