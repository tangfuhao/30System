...
void HariMain(void)
{
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	buf_back  = (unsigned char *) memman_alloc_4k(memman, binfo->scrnx * binfo->scrny * (binfo->vmode >> 3)); /* ココ */
	sheet_setbuf(sht_back, buf_back, binfo->scrnx, binfo->scrny, -1); /* 透明色なし */
	if (binfo->vmode == 8) { /* ココ */
		init_screen8(fat, buf_back, binfo->scrnx, binfo->scrny);
	} else if (binfo->vmode == 16) { /* ココ */
		init_screen16(fat, (unsigned short *) buf_back, binfo->scrnx, binfo->scrny);
	}
	...
	buf_mouse = (unsigned char *) memman_alloc_4k(memman, 256 * (binfo->vmode >> 3)); /* ココ */
	sheet_setbuf(sht_mouse, buf_mouse, 16, 16, 99);
	if (binfo->vmode == 8) { /* ココ */
		init_mouse_cursor8(buf_mouse, 99);
	} else if (binfo->vmode == 16) { /* ココ */
		init_mouse_cursor16((unsigned short *) buf_mouse, 99);
	}
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	unsigned char *buf = (unsigned char *) memman_alloc_4k(memman, 256 * 165 * (binfo->vmode >> 3)); /* ココ */
	sheet_setbuf(sht, buf, 256, 165, -1); /* 透明色なし */
	if (binfo->vmode == 8) {
		make_window8(buf, 256, 165, "console", 0);
		make_textbox8(sht, 8, 28, 240, 128, COL8_000000);
	} else if (binfo->vmode == 16) {
		make_window16((unsigned short *) buf, 256, 165, "console", 0);
		make_textbox16(sht, 8, 28, 240, 128, COL16_000000);
	}
...
void close_console(struct SHEET *sht)
{
	...
	memman_free_4k(memman, (int) sht->buf, 256 * 165 * (binfo->vmode >> 3)); /* ココ */
	sheet_free(sht);
	close_constask(task);
	return;
}
