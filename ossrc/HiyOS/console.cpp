		...
		if (fifo32_status(&task->fifo) == 0) {
			...
			if (i == 3) {	/* カーソルOFF */
				if (cons.sht != 0) {
					...
					} else if (binfo->vmode == 16) { /* ココ */
						boxfill16((unsigned short *) cons.sht->buf, cons.sht->bxsize, COL8_000000,
							cons.cur_x, cons.cur_y, cons.cur_x + 7, cons.cur_y + 15);
					}
				}
				cons.cur_c = -1;
			...
			if (cons.sht != 0) {
				if (cons.cur_c >= 0) {
					if (binfo->vmode == 8) { /* ココ */
						...
						if (cons.cur_c == COL8_FFFFFF) {
							boxfill16((unsigned short *) cons.sht->buf, cons.sht->bxsize, 0xffff, 
								cons.cur_x, cons.cur_y, cons.cur_x + 7, cons.cur_y + 15);
						} else {
							boxfill16((unsigned short *) cons.sht->buf, cons.sht->bxsize, 0x0000, 
								cons.cur_x, cons.cur_y, cons.cur_x + 7, cons.cur_y + 15);
						}
					}
				}
	...
	if (s[0] == 0x09) {	/* タブ */
		for (;;) {
			if (cons->sht != 0) {
				if (binfo->vmode == 8) { /* ココ */
					...
					putfouts16_asc_sht(cons->sht, cons->cur_x, cons->cur_y, COL16_FFFFFF, COL16_000000, " ", 1);
				}
			}
			cons->cur_x += 8;
			if (cons->cur_x == 8 + 240) {
		...
		if (cons->sht != 0) {
			if (binfo->vmode == 8) { /* ココ */
				...
				putfouts16_asc_sht(cons->sht, cons->cur_x, cons->cur_y, COL16_FFFFFF, COL16_000000, s, 1);
			}
		}
		if (move != 0) {
			/* moveが0のときはカーソルを進めない */
		...
		if (sheet != 0) {
			for (y = 28; y < 28 + 112; y++) {
				for (x = 8; x < 8 + 240; x++) {
					if (binfo->vmode == 8) { /* ココ */
						...
						buf[x + y * sheet->bxsize] = buf[x + (y + 16) * sheet->bxsize];
					} else if (binfo->vmode == 16) { /* ココ */
						unsigned short* buf = (unsigned short*) sheet->buf;
						buf[x + y * sheet->bxsize] = buf[x + (y + 16) * sheet->bxsize];
					}
				}
			}
			for (y = 28 + 112; y < 28 + 128; y++) {
				for (x = 8; x < 8 + 240; x++) {
					if (binfo->vmode == 8) { /* ココ */
						unsigned char* buf = sheet->buf;
						buf[x + y * sheet->bxsize] = COL8_000000;
					} else if (binfo->vmode == 16) { /* ココ */
						unsigned short* buf = (unsigned short*) sheet->buf;
						buf[x + y * sheet->bxsize] = COL16_000000;
					}
				}
			}
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		if (binfo->vmode == 8) { /* ココ */
			...
			putfonts16_asc((unsigned short *) sht->buf, sht->bxsize, esi, edi, eax, (char *)(ebp + ds_base));
		}
		if ((ebx & 1) == 0) {
		...
		if (binfo->vmode == 8) { /* ココ */
			...
			boxfill16((unsigned short *) sht->buf, sht->bxsize, ebp, eax, ecx, esi, edi);
		}
		if ((ebx & 1) == 0) {
