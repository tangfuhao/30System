...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		if(strcmp((char *)eax + ds_base, "COM1") == 0)
			reg[7] = open_rs232(&COM1, task);
		else if(strcmp((char *)eax + ds_base, "COM2") == 0)
			reg[7] = open_rs232(&COM2, task);
		else
			reg[7] = -2;
	} else if (edx == 29) {
		if(strcmp((char *)eax + ds_base, "COM1") == 0)
			reg[7] = close_rs232(&COM1, task);
		else if(strcmp((char *)eax + ds_base, "COM2") == 0)
			reg[7] = close_rs232(&COM2, task);
		else
			reg[7] = -2;
	} else if (edx == 30) {
		if(strcmp((char *)eax + ds_base, "COM1") == 0)
			reg[7] = rs232_put(&COM1, (char)(ecx & 0xff), task);
		else if(strcmp((char *)eax + ds_base, "COM2") == 0)
			reg[7] = rs232_put(&COM2, (char)(ecx & 0xff), task);
		else
			reg[7] = -2;
	} else if (edx == 31) {
		if(strcmp((char *)eax + ds_base, "COM1") == 0)
			reg[7] = rs232_get(&COM1, task);
		else if(strcmp((char *)eax + ds_base, "COM2") == 0)
			reg[7] = rs232_get(&COM2, task);
		else
			reg[7] = -2;
	}
	return 0;
