...
#define POKON_VERSION "pokon48"

#define POKO_VERSION "Heppoko-shell \"poko\" version 2.8\n    Copyright (C) 2004 OSASK Project\n"
#define POKO_PROMPT "\npoko>"
...
static struct STR_VIEWER viewers[MAX_VIEWERS + 1] = {
	{ "***", "BEDITC00BIN" },
...
struct STR_BANK *run_viewer(struct STR_VIEWER *viewer, struct SGG_FILELIST *fp, struct SGG_FILELIST *fp2);
void runfile(struct SGG_FILELIST *fp, const char *name);
...
static unsigned char setorder_fn[12];
static int setorder_sg[4];
static unsigned int CFIDE_name[3] = { 0x4d494446, 0x30454741, 0x004e4942 };
...
lib_putstring_ASCII(0x0000, 0, 0, &selwin0[0].subtitle.tbox, 0, 0, "debug!(2)");
		...
		case JOB_CHANGE_DRIVE:
			...
			if (need_wb == 0) {
				sgg_execcmd0(0x009c, 24, 0x190, pjob->param[0], pjob->param[1], pjob->param[2], 0x0000); /* 本当はこの方法は良くない */
		...
		case JOB_CHANGE_DEVICE:
			i = readjob();
			if (need_wb == 0) {
				sgg_execcmd0(0x009c, 16, 0x194, i, 0x0000); /* 本当はこの方法は良くない */
					/* アクセスが競合するかもしれないから、本当ならシステムタスクを経由するべき */
				#if (defined(PCAT))
					if (0x0200 <= i && i <= 0x0208) { /* PC-card reset */
...
void poko_exec_cmd(const char *p)
{
	...
	if (*p != '\0' && *p != '/') {
		...
		/*	poko_mousespeed,	"mousespeed", 10, 1, */
			...
			poko_setorder,		"setorder", 8, 2,
			poko_setimgname,	"setimgname", 10, 1,
			#if (defined(TOWNS))
...
listup:
				...
				default:
					if (SIGNAL_DISK_CHANGE0 <= sig && sig <= SIGNAL_DISK_CHANGE0 + 9) {
						((char *) CFIDE_name)[7] = (sig - SIGNAL_DISK_CHANGE0) | '0';
						writejob_n(5,
							JOB_CHECK_WB_CACHE,
							JOB_CHANGE_DRIVE, CFIDE_name[0], CFIDE_name[1], CFIDE_name[2]
						);
						break;
					}
...
struct STR_BANK *run_viewer(struct STR_VIEWER *viewer, struct SGG_FILELIST *fp, struct SGG_FILELIST *fp2)
/* fp  : viewerもしくはbinのfp。分からなければNULLでもいい。 */
/* fp2 : これがNULLだとorderなし */ 
{
	int i;
	struct STR_JOBLIST *pjob = &job;
	struct FILEBUF *fbuf;
...
//	bank->name[11] = '\0';
	...
	if (fp2) {
		if (pjob->free >= 10) {
			/* 空きが十分にある */
			writejob_n(6, JOB_VIEW_FILE, (int) fp, (int) fbuf, (int) bank, 1 /* num */, (int) fp2);
			writejob_np(4, &viewer->signal[0]);
			return bank;
		}
	} else {
		writejob_n(4, JOB_LOAD_FILE_AND_EXECUTE, (int) fp, (int) fbuf, (int) bank);
		return bank;
	}
	fbuf->linkcount = 0;
error:
...
/* nameにはピリオドを含まない */
{
	...
	if (j == ('B' | 'I' << 8 | 'N' << 16)) {
		struct STR_VIEWER viewer;
		for (i = 0; i < 11; i++)
			viewer.binary[i] = name[i];
		viewer.binary[11] = '\0';
		for (i = 0; i < 4; i++)
			viewer.signal[i] = setorder_sg[i];
		run_viewer(&viewer, fp, searchfid1(setorder_fn));
		setorder_fn[0] = '\0';
		goto ret;
	}
	if (j == ('P' | 'S' << 8 | 'F' << 16)) {
...
int poko_setorder(const char *cmdlin)
/* setorder [filename.ext] */
/* 次に使いそうなタスク番号を把握 */
/* それめがけて登録 */
/* いやファイル名丸暗記で十分だろう */
/* ファイル名を書かないと、order取り消し */
{
	setorder_fn[0] = '\0';
	cmdlin = pokosub_getfilename(cmdlin, setorder_fn);
	setorder_fn[ 8] = setorder_fn[ 9];
	setorder_fn[ 9] = setorder_fn[10];
	setorder_fn[10] = setorder_fn[11];
	setorder_fn[11] = '\0';
	return 1;
error:
	return -ERR_ILLEGAL_PARAMETERS;
...
int poko_setimgname(const char *cmdlin)
/* setimgname filename.ext */
{
	CFIDE_name[0] &= 0;
	cmdlin = pokosub_getfilename(cmdlin, (char *) CFIDE_name);
	CFIDE_name[2] >>= 8;
	writejob_n(5,
		JOB_CHECK_WB_CACHE,
		JOB_CHANGE_DRIVE, CFIDE_name[0], CFIDE_name[1], CFIDE_name[2]
...
int osarjc(int siz, UCHAR *p0)
{
	int ret, i;
	unsigned int ti, tf;
	unsigned char tmp[56], *tp;
	ret &= 0;
	if (siz >= 0x20 && (p0[16] == 0x00 || (siz >= 0x40 && p0[0x3f] == 0x00))
		&& *(int *) &p0[8] == ('G' | 'U' << 8 | 'I' << 16 | 'G' << 24)
		&& *(int *) &p0[12] == ('U' | 'I' << 8 | '0' << 16 | '0' << 24)) {
		if (*p0 == 0xf4) {
			/* 定型スタートアップ */
			if (0x01 <= p0[1] && p0[1] <= 0x02) {
				if (p0[0x2] != 0x80) {
					/* 0x10-0x2f部分の変換を解く */
					for (i = 0; i < 6; i++)
						tmp[i] = p0[i + 2];
					for (i = 6; i < 56; i++)
						tmp[i] = p0[i + 10];
					for (i = 0; i < 56; i++)
						tmp[i] = tmp[i] << 1 | ((tmp[i] >> 7) ^ 0x01);
					tp = tmp;
					tf = gets7s(&tp);
					ti = gets7s(&tp) + 2;
					*(int *) &p0[0x10] = ti << 12 | ti >> 20; /* stack+work */
					ti = gets7s(&tp) + 4;
					*(int *) &p0[0x20] = ti << 10 | ti >> 22; /* stack */
					ti = gets7s(&tp) + 1;
					*(int *) &p0[0x24] = ti <<  4 | ti >> 28; /* .data */
					ti = gets7s(&tp) + 4;
					*(int *) &p0[0x2c] = ti <<  4 | ti >> 28; /* img-location */
					if (tf & 1)
						*(int *) &p0[0x14] = gets7s(&tp);
					if (tf & 2) {
						ti = gets7s(&tp) + 1;
						*(int *) &p0[0x28] = ti <<  4 | ti >> 28; /* bss */
					}
				}
				if (p0[1] == 0x01) {
					p0[0x04] = 0x6a;
					p0[0x05] = 0xff;
					*(int *) &p0[0x30] = 0xf16a206a;
					*(int *) &p0[0x34] = 0xe389046a;
					*(int *) &p0[0x38] = 0x0000009a;
					*(int *) &p0[0x3c] = 0x8300c700;
					p0[0x40] = 0xc4;
					p0[0x41] = 0x10;
				} else {
					p0[0x04] = 0x6a;
					p0[0x05] = 0x21;
					*(int *) &p0[0x30] = 0x046af16a;
					*(int *) &p0[0x34] = 0x009ae389;
					*(int *) &p0[0x38] = 0xc7000000;
					*(int *) &p0[0x3c] = 0x0cc48300;
				}
				*(int *) &p0[0x00] = 0x20628b2e;
				p0[0x06] = 0xeb;
				p0[0x07] = 0x28;
				ret |= 1;
			}
