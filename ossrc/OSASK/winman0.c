...
#if (defined(PCAT))
	int SWORK_VGA_mode, SWORK_VBE_12, SWORK_VBE_10, SWORK_VBE_28;
#endif
	...
	*(pjob->rp = pjob->wp = pjob->list) = 0; /* たまった仕事はない */
	...
	for (;;) {
		...
		case 0x0018:
			...
			fromboot = signal[1];
			#if (defined(PCAT))
				SWORK_VGA_mode = signal[2];
				SWORK_VBE_12 = signal[3];
				SWORK_VBE_10 = signal[4];
				SWORK_VBE_28 = signal[5];
				siglen += 5;
				if (fromboot == 0 && (SWORK_VGA_mode & 0xffff) == 0xffff) {
					/* VESAの調査を開始 */
...
void job_setvgamode2()
{
	struct WM0_WINDOW *win;
		...
		if ((SWORK_VGA_mode & 0xffff) != 0xffff) {
			/* 画面モードによらず結果を供給 */
			int x2y2[2], drv = 0;
			if (SWORK_VGA_mode == 0) {
				/* ハードウェア制御による切り替え */
			...
			mode = job.int0 = SWORK_VGA_mode & 0xffff;
			if (mode <= 0x7f) {
				/* 0x12か0x6a */
				/* linear = 0を伝達 */
				...
				screen.x2 =  SWORK_VBE_12        & 0xffff;
				screen.y2 = (SWORK_VBE_12 >> 16) & 0xffff;
				/* linearを伝達 */
				sgg_execcmd0(0x0088, 0x0001, SWORK_VBE_28, 0x0000);
				x2y2[0] = screen.x2;
				x2y2[1] = screen.y2 - (RESERVELINE0 + RESERVELINE1);
				sgg_execcmd0(0x00b0, 0, 2, 0, x2y2, 0x000c, 0x0000); /* set info */
				if ((SWORK_VGA_mode >> 16) == 0x0408)
					drv = 1;
				if ((SWORK_VGA_mode >> 16) == 0x0610)
					drv = 2;
				if ((SWORK_VGA_mode >> 16) == 0x0620)
					drv = 3;
			}
