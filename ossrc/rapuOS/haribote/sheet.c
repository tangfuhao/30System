	...
	for (h = h0; h <= h1; h++) {
		...
		if (binfo->vmode == 8) {
			...
			if ((sht->vx0 & 2) == 0) {
				...
				for (by = by0; by < by1; by++) {
					vy = sht->vy0 + by;
					for (bx = bx0; bx < bx1 && (bx & 1) != 0; bx++) {	/* 前の端数を1バイトずつ */
						vx = sht->vx0 + bx;
						if (map[vy * ctl->xsize + vx] == sid) {
							*(unsigned short *)	&vram[(vy * ctl->xsize + vx) * 2] =
								buf[by * sht->bxsize + bx];
						}
					}
					vx = sht->vx0 + bx;
					o = (short *) &map[vy * ctl->xsize + vx];
					q = (int   *) &vram[(vy * ctl->xsize + vx) * 2];
					r = (int   *) &buf[by * sht->bxsize + bx];
					for (i = 0; i < i1; i++) {							/* 2の倍数部分 */
						if (o[i] == sid2) {
							q[i] = r[i];
						} else {
							bx2 = bx + i * 2;
							...
							if (map[vy * ctl->xsize + vx + 0] == sid) {
								*(unsigned short *)	&vram[(vy * ctl->xsize + vx + 0) * 2] =
									buf[by * sht->bxsize + bx2 + 0];
							}
							if (map[vy * ctl->xsize + vx + 1] == sid) {
								*(unsigned short *)	&vram[(vy * ctl->xsize + vx + 1) * 2] =
									buf[by * sht->bxsize + bx2 + 1];
							}
						}
					}
					for (bx += i1 * 2; bx < bx1; bx++) {				/* 後の端数を1バイトずつ */
						vx = sht->vx0 + bx;
						if (map[vy * ctl->xsize + vx] == sid) {
							*(unsigned short *)	&vram[(vy * ctl->xsize + vx) * 2] =
								buf[by * sht->bxsize + bx];
...
struct SHEET *sheet_view(struct SHTCTL *ctl)
{
	struct SHEET *sht;
	int i;
	for (i = 0; i < MAX_SHEETS; i++) {
		if (ctl->sheets0[i].height == -100) {
			sht = &ctl->sheets0[i];
			sheet_updown(sht, ctl->top);
			return sht;
		}
	}
	return 0;	/* 最小化されたのシートがなかった */
}
