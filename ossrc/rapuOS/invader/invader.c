...
void putstr(int win, short *winbuf, int x, int y, int col, unsigned char *s);
	...
	for (;;) {
		...
		if (c != ' ') {
			if ('a' <= c && c <= 'h') {
				...
				q += x;
				for (i = 0; i < 16; i++) {
					if ((p[i] & 0x80) != 0) { q[0] = coltbl[col]; }
					if ((p[i] & 0x40) != 0) { q[1] = coltbl[col]; }
					if ((p[i] & 0x20) != 0) { q[2] = coltbl[col]; }
					if ((p[i] & 0x10) != 0) { q[3] = coltbl[col]; }
					if ((p[i] & 0x08) != 0) { q[4] = coltbl[col]; }
					if ((p[i] & 0x04) != 0) { q[5] = coltbl[col]; }
					if ((p[i] & 0x02) != 0) { q[6] = coltbl[col]; }
					if ((p[i] & 0x01) != 0) { q[7] = coltbl[col]; }
					q += 336;
				}
				q -= 336 * 16 + x;
			} else {
